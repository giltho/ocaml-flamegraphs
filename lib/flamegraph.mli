...
type stack = {
  frames : frame list;
      (** Stack frames ordered bottom-to-top (caller to callee). The first
          element is the root/entry point, the last is the leaf. *)
  weight : float;
...
(** The flamegraph data structure.
    ...
    Internally represented as a prefix tree (trie) for efficient merging of
    common stack prefixes. *)

type node = {
  frame : frame;  (** The frame at this node. *)
  self_weight : float;  (** Weight of stacks ending at this frame. *)
  total_weight : float;  (** Total weight including all descendants. *)
  children : node list;  (** Child nodes (callees). *)
}
...
(** [add_stack stack fg] adds a stack trace to the flamegraph. Stacks with the
    same frame sequence are merged by summing weights. *)

val add_stacks : stack list -> t -> t
(** [add_stacks stacks fg] adds multiple stack traces. Equivalent to
...
(** [stack frames] creates a stack trace with weight [1.0]. Frames should be
    ordered bottom-to-top (caller to callee).
    @param weight The weight/count for this stack. Default is [1.0]. *)

val stack_of_strings : ?weight:float -> string list -> stack
(** [stack_of_strings names] creates a stack from simple function names. Each
    string becomes a frame with no metadata.
    @param weight The weight/count for this stack. Default is [1.0]. *)

(** {2 Tree-based Construction}

    An alternative API for building flamegraphs using an explicit tree
    structure. This is often more natural when generating flamegraphs
    programmatically.
...
type tree
(** A tree node for constructing flamegraphs. Opaque type - use {!node} to
    construct. *)
...
(** [node name children] creates a tree node.
    ...
    @param weight
      Self-weight at this node (time spent in this frame itself, not in
      children). Default is [0.0]. *)

val of_tree : tree -> t
(** [of_tree tree] creates a flamegraph from a single tree. The tree's root
    becomes the flamegraph's root. *)

val of_trees : tree list -> t
(** [of_trees trees] creates a flamegraph from multiple root trees. Use this
...
(** [iter f fg] calls [f stack_frames self_weight] for each unique stack in the
    flamegraph where self_weight > 0. *)
