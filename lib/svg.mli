(** SVG rendering for flamegraphs.
    ...
    This module provides functions to render flamegraphs as interactive
    SVG images with zoom, pan, and search functionality. *)
...
type color_scheme =
  | Hot
      (** Red-orange-yellow gradient, suitable for CPU profiles. *)
  | Cold
      (** Blue-cyan gradient, suitable for off-CPU or IO profiles. *)
  | Memory
      (** Green gradient, suitable for memory allocation profiles. *)
  | Io
      (** Purple gradient, suitable for IO profiles. *)
  | Custom of (Flamegraph.frame -> depth:int -> string)
      (** User-provided function returning a CSS color string.
          Receives the frame and its depth in the stack (0 = root). *)
...
type config = {
  ...
  row_height : int;
      (** Height of each frame row in pixels. Default: [16]. *)
  font_size : int;
  ...
  color_scheme : color_scheme;
      (** Color scheme for frames. Default: [Hot]. *)
  title : string option;
      (** Optional title displayed at the top. Default: [None]. *)
  background : string;
      (** Background color. Default: ["#f8f8f8"]. *)
}
...
(** [to_file ?config path fg] writes the SVG to a file.
