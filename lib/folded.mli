(** Folded stacks format (compatible with flamegraph.pl).
    ...
    This module provides import and export functionality for the
    folded stacks format used by Brendan Gregg's flamegraph.pl tool.

    The format is: [frame1;frame2;frame3 weight\n]

    Example:
    {[
      main;process;compute 150.5
      main;process;allocate 42.0
      main;init 30.0
    ]} *)
...
(** [to_file ?config path fg] writes folded stacks to a file.
    Returns [Error msg] if the file cannot be written. *)

(** {1 Import} *)

val of_string : ?config:config -> string -> (Flamegraph.t, string) result
(** [of_string ?config s] parses folded stacks format into a flamegraph.
    Returns [Error msg] if parsing fails. *)
