...
type frame = { name : string; metadata : (string * string) list }
...
type t = { roots : node list; total_weight : float }
  ...
  | frame :: rest ->
      ...
      let rec update_or_append = function
        | [] ->
            ...
            [ { frame; self_weight; total_weight = weight; children } ]
        | node :: tail when node.frame.name = frame.name ->
            (* Update existing node, keep position *)
            let self_weight =
              if rest = [] then node.self_weight +. weight else node.self_weight
            in
            let children = insert_into_nodes rest weight node.children in
            {
              node with
              self_weight;
              total_weight = node.total_weight +. weight;
              children;
            }
            :: tail
        | node :: tail -> node :: update_or_append tail
      in
...
let add_stacks stacks fg = List.fold_left (fun fg s -> add_stack s fg) fg stacks
...
let depth fg = List.fold_left (fun m n -> max m (node_depth n)) 0 fg.roots
...
type tree =
  | Node of { t_frame : frame; t_weight : float; t_children : tree list }

let node ?(metadata = []) ?(weight = 0.0) name children =
  Node
    { t_frame = { name; metadata }; t_weight = weight; t_children = children }
...
let rec tree_to_node (Node { t_frame; t_weight; t_children }) =
  ...
  let children_weight =
    List.fold_left (fun acc (n : node) -> acc +. n.total_weight) 0.0 child_nodes
  in
  {
    frame = t_frame;
...
let of_trees trees =
  ...
  let total =
    List.fold_left (fun acc (n : node) -> acc +. n.total_weight) 0.0 root_nodes
  in
